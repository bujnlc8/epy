...
class HTMLtoLines(HTMLParser):
    ...
    def __init__(self, sects={""}, scale_times:int=0):
    ...
    def get_structured_text(
        ...
        for n, line in enumerate(self.text):
            ...
            if n in self.idhead:
                ...
                text += [line.center(textwidth * self.scale_times)] + [""]
                formatting += [
                    InlineStyle(
        ...
        # chapter suffix
        text += ["***本章结束***".center(textwidth * self.scale_times)]
...
def parse_html(
    ...
    *,
    ...
    parser = HTMLtoLines(section_ids, scale_times=scale_times)
    # try:
