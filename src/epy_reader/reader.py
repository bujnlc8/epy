...
class Reader:
    def __init__(self, screen, ebook: Ebook, config: Config, state: State, scale_times=0):
        ...
        # scale times
        self.scale_times = int(os.environ.get('EPY_SCALE_TIMES', '2')) if scale_times <= 0 else scale_times
    ...
    def run_counting_letters(self):
        if self._multiprocess_support:
            try:
                ...
                self._process_counting_letter = multiprocessing.Process(
                    ...
                    args=(self.ebook, self._proc_child, self.scale_times),
                )
                # forking will raise
        ...
        if not self._multiprocess_support:
            self.letters_count = count_letters(self.ebook, self.scale_times)
                ...
                if k in tuple(Key(i) for i in range(48, 58)):  # i.e., k is a numeral
                    ...
                    if k in self.keymap.Quit:
                        ...
                        if totlines - reading_state.row > rows * self.spread:
                            self.page_animation = Direction.FORWARD
                            reading_state = dataclasses.replace(
                                reading_state, row=reading_state.row + (rows * self.spread) - 1,
                            )
                    ...
                    #     continue
                        ...
                        if countstring == "":
                            default_width = int(120 / self.scale_times)
                            # if called without a count, toggle between 60 cols and full width
                            if reading_state.textwidth != default_width and (cols - 2) // self.scale_times >= default_width:
                                return ReadingState(
                                    content_index=reading_state.content_index,
                                ...
                                return ReadingState(
                                    content_index=reading_state.content_index,
                                    textwidth=int((cols - 2) // self.scale_times),
                                    row=reading_state.row,
                                    rel_pctg=reading_state.row / totlines,
                                )
                        else:
                        ...
                        elif reading_state.textwidth >= int((cols - 2) // self.scale_times):
                            reading_state = dataclasses.replace(reading_state, textwidth=int((cols - 2) // self.scale_times))
...
def start_reading(stdscr, filepath: str, scale_times: int):

    ebook = get_ebook_obj(filepath)
    state = State()
    config = Config()

    reader = Reader(screen=stdscr, ebook=ebook, config=config, state=state, scale_times=scale_times)
