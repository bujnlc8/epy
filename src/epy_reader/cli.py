...
def get_matching_library_item(state: State, pattern: str, threshold: float = 0.5) -> Optional[LibraryItem]:
    ...
    for item in library_items:
        ...
        match_value = sum([i.size for i in SM(None, tomatch.lower(), pattern.lower()).get_matching_blocks()]
                          ) / float(len(pattern))
        matches.append((
            item,
            match_value,
...
def parse_cli_args() -> argparse.Namespace:
    ...
    args_parser = argparse.ArgumentParser(
        prog=prog,
        usage=f"%(prog)s [-h] [-r] [-d] [-v] [-s] {positional_arg_help_str}",
    ...
    args_parser.add_argument(
        ...
        default=0,
        type=int,
        help='the scale times, if read chinese book, set it to 2, else 1',
    )
    args_parser.add_argument(
        "-v",
        "--version",
...
def find_file() -> Tuple[str, bool, int]:
    args = parse_cli_args()
    state = State()
    ...
    if len(args.ebook) == 0:
        ...
        if last_read:
            return last_read, args.dump, args.scale
        else:
        ...
        if nth is not None:
            ...
            if file:
                return file.filepath, args.dump, args.scale
            else:
                print(f"ERROR: #{nth} file not found.")
                print_reading_history(state)
                sys.exit(1)
        elif is_url(args.ebook[0]):
            return args.ebook[0], args.dump, args.scale
    ...
    if match:
        return match.filepath, args.dump, args.scale
    else:
        sys.exit("ERROR: Found no matching ebook from history.")


def dump_ebook_content(filepath: str, scale_times:int) -> None:
    ...
    try:
        ...
        for i in ebook.contents:
            ...
            src_lines = parse_html(content, scale_times=scale_times)
