...
def count_letters(ebook: Ebook, scale_times:int=0) -> LettersCount:
...
def count_letters_parallel(ebook: Ebook, child_conn, scale_times:int) -> None:
    child_conn.send(count_letters(ebook, scale_times=scale_times))
    child_conn.close()
